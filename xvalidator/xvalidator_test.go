...
func TestLocaleTranslatorsAndTranslationRegisterFunc(t *testing.T) {
	for _, tc := range []struct {
		...
		{ItLocaleTranslator(), "it", "domenica", "gennaio"},
		...
		{ViLocaleTranslator(), "vi", "Chủ Nhật", "tháng 1"},
	...
	for _, tc := range []struct {
		...
		{ItLocaleTranslator(), ItTranslationRegisterFunc(), "{0} è un campo obbligatorio", "{0} deve essere diverso da {1}"},
		...
		{ViLocaleTranslator(), ViTranslationRegisterFunc(), "{0} không được bỏ trống", "{0} không được bằng {1}"},
