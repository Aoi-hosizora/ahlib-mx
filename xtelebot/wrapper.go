...
// BotWrapper represents a telebot.Bot wrapper type with some custom handling and sending behaviors. For more telegram bot api details,
...
// IsHandled checks whether given endpoint's handler has been handled or registered, panics when using invalid endpoint (not string and telebot.CallbackEndpoint).
...
// RemoveHandler removes the handler of given endpoint, panics when using invalid endpoint (not string and telebot.CallbackEndpoint).
...
// HandleCommand handles string command with MessageHandler to telebot.Bot, visit https://github.com/tucnak/telebot/tree/v2#commands for more details.
...
// HandleReplyButton handles telebot.ReplyButton with MessageHandler to telebot.Bot, visit https://github.com/tucnak/telebot/tree/v2#keyboards for more details.
...
// HandleInlineButton handles telebot.InlineButton with CallbackHandler to telebot.Bot, visit https://github.com/tucnak/telebot/tree/v2#keyboards for more details.
...
// RespondEventType is a type of respond event type, such as "send", "reply" and "edit", used in respondedCallback.
type RespondEventType string

const (
	RespondSendEvent   RespondEventType = "send" // RespondEventType for BotWrapper.RespondSend.
	RespondReplyEvent  RespondEventType = "rep"  // RespondEventType for BotWrapper.RespondReply.
	RespondEditEvent   RespondEventType = "edit" // RespondEventType for BotWrapper.RespondEdit.
	RespondDeleteEvent RespondEventType = "del"  // RespondEventType for BotWrapper.RespondDelete.
)

// RespondEvent is a type of respond event, containing respond method arguments and responded results, used in respondedCallback.
...
// RespondSend responds and sends message to given telebot.Chat and invokes responded callback.
...
// RespondReply responds and replies message to given telebot.Message explicitly or implicitly and invokes responded callback.
func (b *BotWrapper) RespondReply(source *telebot.Message, explicit bool, what interface{}, options ...interface{}) (*telebot.Message, error) {
	...
	if !explicit {
		...
		msg, err = b.bot.Reply(source, what, options...)
	}
	if err == telebot.ErrUnsupportedWhat {
		return nil, err
	}
...
// RespondEdit responds and edits given message with value and invokes responded callback.
...
// RespondDelete responds and deletes given message and invokes responded callback.
func (b *BotWrapper) RespondDelete(source *telebot.Message) error {
	...
	if b.respondedCallback != nil {
		b.respondedCallback(RespondDeleteEvent, &RespondEvent{
			DeleteSource: source,
			DeleteResult: source /* fake, for unifying */, ReturnedError: err,
		})
	}
	return err
}
...
// SetHandledCallback sets endpoint handled callback, will be invoked in handling methods, defaults to print debug message.
func (b *BotWrapper) SetHandledCallback(f func(endpoint interface{}, formattedEndpoint string, handlerName string)) {
	b.handledCallback = f
}

// SetReceivedCallback sets received callback, will be invoked after receiving consuming messages which are handled, defaults to do nothing.
func (b *BotWrapper) SetReceivedCallback(cb func(endpoint interface{}, received *telebot.Message)) {
	b.receivedCallback = cb
}

// SetRespondedCallback sets responded callback, will be invoked in respond methods, defaults to do nothing.
func (b *BotWrapper) SetRespondedCallback(cb func(typ RespondEventType, event *RespondEvent)) {
	b.respondedCallback = cb
}

// SetPanicHandler sets panic handler for all endpoint handlers, defaults to print warning message only with given panicked `value`.
